...
/*   Updated: 2025/05/25 22:27:17 by ehossain         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_calculate_move_cost(t_stack *stack_a, t_stack *stack_b)
{
	...
	while (current_node)
	{
		current_target = ft_find_node_by_index(stack_a, current_node->i_target);
		...
		t_stack *target_node, t_stack *cheapest_node)
{
	...
	t_stack	*tmp_b;
	t_stack	*tmp_target;
	t_stack	*tmp_cheapest;

	tmp_a = *stack_a;
	tmp_b = *stack_b;
	tmp_target = target_node;
	tmp_cheapest = cheapest_node;
	// Loop until the top of stack_a is the target_node.
	while (tmp_a != tmp_target && tmp_target)
	{
		// Rotate stack_a until the 'target_node' is at the top.
		if (tmp_target->close_to_top)
			ft_ra(stack_a, 1);
		else
			ft_rra(stack_a, 1);
	...
	// Loop until the top of stack_b is the cheapest_node.
	while (tmp_b != tmp_cheapest && tmp_cheapest)
	{
		// Rotate stack_b until the 'cheapest_node' is at the top.
		if (tmp_cheapest->close_to_top)
			ft_rb(stack_b, 1);
		else
			ft_rrb(stack_b, 1);
		tmp_cheapest = tmp_cheapest->next;
		ft_positioning(*stack_b);
	}
}
